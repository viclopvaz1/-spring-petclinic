...
public interface SpringDataCausaRepository extends CausaRepository, Repository<Causa, Integer> {
	...
	@Override
	@Query("SELECT * FROM Causa causa WHERE causa.id IN(SELECT donacion.causa_id FROM Donacion donacion WHERE donacion.persona_username LIKE :username%)")
	Collection<Causa> causesWhereIDonated(@Param("username") String username);
